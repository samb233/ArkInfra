...
const (
	// 默认生产力
	DefaultProductivity int = 1

	// 默认每分钟消耗疲劳
	DefaultFtgPerMinute int = 100

	// 初始疲劳
	DefaultFtg int = 144000
)
...
// 新建工人
func NewWorker(id, ftg int, options ...WorkerOptions) *Worker {
	worker := &Worker{
		ID:  id,
		Ftg: ftg,
	}
...
// ftg: 奖励疲劳
func BonusOptions(itemPrefix string, bonusProductivity, bonusFtg int) WorkerOptions {
...
// stroage: 奖励库存
func StorageOptions(roomTypePrefix string, bonusStorage int) WorkerOptions {
	return func(w *Worker) {
...
// 应当在进驻房间的时候调用
// roomType: 房间类型
func (w *Worker) GetBonusStorage(roomType int) int {
	roomTypeString := strconv.Itoa(roomType)
...
// itemID: 物品ID
func (w *Worker) GetBonusProductivity(itemID int) int {
...
// production: 产出的生产力
// TODO: 优化计算
func (w *Worker) Produce(itemID int, minutes int) (workingMinutes int, production int) {
	// 判断是否疲劳
	if w.IsExhausted() {
		return 0, 0
	}
...
func (w *Worker) CheckBonus(itemID int) (productivity int, ftgPerMinute int) {
...
func (w *Worker) CheckWorkingTime(minutes int, ftgPerMinute int) int {
